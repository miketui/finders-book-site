 ...
 */
export function buildGa4Payload(name, {
  transactionId, amountMinorUnits, currency, items = [], clientId, sessionId,
} = {}) {
  const suppliedClientId = typeof clientId === 'string' ? clientId.trim() : '';
  const validClientId = /^\d+\.\d+$/.test(suppliedClientId) ? suppliedClientId : '';
  const suppliedSessionId = String(sessionId ?? '').trim();
  const validSessionId = /^\d+$/.test(suppliedSessionId) && Number(suppliedSessionId) > 0
    ? suppliedSessionId
    : '';
  const params = {
  ...
  if (validSessionId) params.session_id = validSessionId;
  return {
    // Prefer the consented browser id when Payhip echoes it back. Older/direct
    // orders retain the privacy-safe per-transaction synthetic fallback.
    client_id: validClientId || ga4ClientId(transactionId),
