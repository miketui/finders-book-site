...
| GA4 paid attribution + live revenue | ⚠️ **revenue proven; attribution remediation in code** | The 2026-08-18 controlled discounted Ultimate purchase is processed in standard GA4 reporting as exactly 1 purchase / 1 transaction / **$2 USD** purchase and total revenue. Its original server event used a synthetic transaction-derived client id and no session id, so processed `sessionSourceMedium` and `sessionCampaignName` are `(not set)`. The remediation now uses Payhip direct-checkout metadata to carry the consented browser GA `client_id` + `session_id` into the webhook and Measurement Protocol purchase, retaining the synthetic fallback for non-consented/legacy orders. Close C04 only after one future real or safely authorized test checkout proves processed source/medium/campaign attribution. |
